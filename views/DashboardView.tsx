...
const DashboardView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      setPipelineData(getPipelineMetrics());
    };
    fetchData();
      ...
      <Card title="Pipeline Latency (ms)" className="lg:col-span-2">
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={pipelineData}>
            ...
            <Legend />
            <Line type="monotone" dataKey="ingest" stroke="#00aaff" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
            <Line type="monotone" dataKey="enrich" stroke="#ff4081" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
            <Line type="monotone" dataKey="query" stroke="#4caf50" strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 8 }} />
          </LineChart>
        </ResponsiveContainer>
      </Card>
      ...
      <Card title="Data Throughput (events/min)" className="lg:col-span-3">
        <ResponsiveContainer width="100%" height={300}>
            <BarChart data={pipelineData}>
                ...
                <XAxis dataKey="time" stroke="#9a9a9a" />
                <YAxis stroke="#9a9a9a" />
                <Tooltip contentStyle={{ backgroundColor: '#2d2d2d', border: '1px solid #4a4a4a' }} cursor={{fill: '#2d2d2d'}}/>
                <Legend />
                <Bar dataKey="throughput" fill="#00aaff" />
            </BarChart>
        </ResponsiveContainer>
      </Card>
